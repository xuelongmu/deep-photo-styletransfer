...
Dependencies:
* [Torch](https://github.com/torch/torch7) (with [csvigo](https://github.com/clementfarabet/lua---csv) and [loadcaffe](https://github.com/szagoruyko/loadcaffe))
...
```
python3 mattinglaplacian.py -dir='/path/to/inputs/' -out='/path/to/outputs/'
```
and then
```
python gen_all.py
...
2. Compute the matting Laplacian matrix using ``gen_laplacian/mattinglaplacian.py``. The output matrix will have the following filename form: ``gen_laplacian/Input_Laplacian_3x3_1e-7_CSR<id>.csv``;
...
* The matting Laplacian matrix Python code was adapted from Anat Levin's Matlab [code](http://www.wisdom.weizmann.ac.il/~levina/matting.tar.gz).
